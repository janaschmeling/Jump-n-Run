...
from menu_config import (BUTTON_WIDTH, BUTTON_HEIGHT, BUTTON_CORNER_RADIUS,
                         ...
                         COLOR_BLUE, COLOR_HIGHLIGHT, COLOR_WHITE,
                         BUTTON_FONT_SIZE)
...
class MenuButton:
    ...
    def update(self, dt, mouse_pos):
        ...
        if self.pressed:
            self.button_bg.color = color.rgb(*COLOR_HIGHLIGHT)
        elif self.hovered:
            self.button_bg.color = color.rgb(*COLOR_HIGHLIGHT)
        else:
            self.button_bg.color = color.rgb(*COLOR_BLUE)
    ...
    def set_active(self, active):
        ...
        self.active = active
        if not active:
            self.button_bg.color = color.rgb(100, 100, 100)
        else:
            self.button_bg.color = color.rgb(*COLOR_BLUE)
